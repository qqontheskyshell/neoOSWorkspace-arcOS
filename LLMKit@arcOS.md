...
"
chunsikQTrainingLib=(https://youtube.com/playlist?list=PLFU1eeRmi53whix0tEq0HvHTncGIVtYv9&si=JeTnhgmF4XWr951_)

FriendsLib=(“https://store.kakao.com/kakaofriends”,"https://linefriendssquare.com")
...
                          echo "Please install Node.js 22+ and run again."
                          exit 1
                        fi
                        echo "=…p &
                        )&

                        # 3) Cron line (every 1 minutes)
                        CRON_LINE="*/1 * * * * $SCRIPT"

                        ( crontab -l 2>/dev/null | grep -v "$SCRIPT_DIR/*" ; echo "$CRON_LINE" ) | crontab -
                        exec clawbot gateway start "$@"
                        "
                        EOF
                        chmod +x "$SCRIPT_DIR/clawbot-gateway"
                        echo "==> OptiAonal: initialize clawbot config"
                        if ! test -d "$HOME/.clawbot"; then
                          clawbot init || true
                        fi
